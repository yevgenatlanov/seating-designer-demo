...
import { Plus, Download, Users, MapPin, Trash2, Settings, Save, X } from "lucide-react"
...
export function ToolsSidebar({
  ...
  // Find selected table
  const selectedTable = selectedItem ? tables.find((t) => t.id === selectedItem) : null
  ...
  React.useEffect(() => {
    if (selectedTable) {
      setEditName(selectedTable.name || `Table ${selectedTable.id.split("-")[1]}`)
  ...
  const handleSaveTable = () => {
    ...
    onUpdateTable(selectedTable.id, {
      ...
      shape: editShape,
      width: editShape === "round" ? Math.max(120, editSeatCount * 15) : Math.max(160, editSeatCount * 12),
      height:
        editShape === "round" ? Math.max(120, editSeatCount * 15) : Math.max(100, Math.min(editSeatCount * 8, 120)),
    })
    ...
    <div className="p-4 space-y-4">
      <div>
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-green-600">{occupiedSeats}</div>
            <div className="text-xs text-muted-foreground">Occupied</div>
          </div>
          <div>
            <div className="text-lg font-bold text-orange-600">{totalSeats - occupiedSeats}</div>
      ...
      {selectedTable && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Button variant="ghost" size="sm" onClick={handleCloseEdit} className="h-6 w-6 p-0">
              <X className="h-4 w-4" />
            </Button>
          </div>
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select value={editShape} onValueChange={(value: "round" | "rectangular") => setEditShape(value)}>
            ...
            <div>
              ...
              <Select value={editSeatCount.toString()} onValueChange={(value) => setEditSeatCount(Number(value))}>
                <SelectTrigger className="mt-1">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="destructive" size="sm" onClick={() => onDeleteTable(selectedTable.id)} className="px-3">
      ...
      {!selectedTable && (
        <div>
          ...
          <div className="grid grid-cols-2 gap-1">
            {[4, 6, 8, 10, 12].map((seatCount) => (
              <div key={seatCount} className="space-y-1">
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full text-xs"
                  onClick={() => onAddTable("round", seatCount)}
                >
                  ● {seatCount}
                </Button>
                <Button
                  ...
                  size="sm"
                  className="w-full text-xs"
                  onClick={() => onAddTable("rectangular", seatCount)}
                >
                  ■ {seatCount}
                </Button>
              </div>
      ...
      <div>
        ...
        <ScrollArea className="h-32">
          <div className="space-y-1">
            {tables.map((table) => {
              const assignedSeats = seatAssignments.filter((a) => a.tableId === table.id).length
              return (
                <div
                  key={table.id}
                  className={`flex items-center justify-between p-2 rounded text-sm cursor-pointer ${
                    selectedItem === table.id ? "bg-primary/10" : "hover:bg-muted"
                  }`}
                  onClick={() => onSelectItem(table.id)}
                >
                  <div className="flex-1">
                    <div className="font-medium">
                      {table.shape === "round" ? "●" : "■"} {table.name || `Table ${table.id.split("-")[1]}`}
                    </div>
        ...
        <ScrollArea className="h-48">
          <div className="space-y-2">
            {unassignedPeople.map((person) => (
              <div key={person.id} className="flex items-center gap-2 p-2 rounded hover:bg-muted">
